...
namespace BtB.WC.Bridge
{
    ...
    public class BridgeEditor : EditorWindow, IHasCustomMenu
    {
        ...
        public ParamsBridge paramsBridge;
        ...
        private void SaveSettings()
        {
            try
            {
                ...
                string dataAsJson = JsonUtility.ToJson(paramsBridge);
        ...
        private void LoadSettings()
        {
            try
            {
                ...
                if (File.Exists(settingsFilePath))
                {
                    ...
                    paramsBridge = JsonUtility.FromJson<ParamsBridge>(dataAsJson);
                }
                else
                {
                    paramsBridge = new ParamsBridge();
                }
            }
            catch (Exception e)
            {
        ...
        public void OnGUI()
        {
            ...
            if (paramsBridge.IsBridgeFileValid())
            {
                if (GUILayout.Button("SYNCHRONIZE", GUILayout.Height(50)))
                {
                    if (!File.Exists(paramsBridge.bridgeFilePath))
                    {
                    ...
                    string terrainFolder = Application.dataPath + "/" + paramsBridge.terrainsFolderName + "/" + paramsBridge.assetName + "/";
                    DirectoryInfo target = new DirectoryInfo(terrainFolder + "Assets");
                    DirectoryInfo source = new DirectoryInfo(paramsBridge.bridgeFilePath).Parent;

                    if (source != null && source.Parent != null)
                        source = new DirectoryInfo(source.FullName + "/Assets/");

                    if (paramsBridge.deleteUnusedAssets && Directory.Exists(@"Assets/" + paramsBridge.terrainsFolderName + "/" + paramsBridge.assetName))
                    {
                        foreach (string filePath in Directory.GetFiles(@"Assets/" + paramsBridge.terrainsFolderName + "/" + paramsBridge.assetName))
                            if (filePath.Contains(paramsBridge.assetName + "_") || filePath.EndsWith(".mat") || filePath.EndsWith(".terrainlayer") || filePath.EndsWith(".png"))
                                AssetDatabase.DeleteAsset(filePath);

                        foreach (string file in Directory.GetFiles(@"Assets/" + paramsBridge.terrainsFolderName + "/" + paramsBridge.assetName + "/Assets/"))
                            AssetDatabase.DeleteAsset(file);

                        foreach (string subdir in Directory.GetDirectories(@"Assets/" + paramsBridge.terrainsFolderName + "/" + paramsBridge.assetName + "/Assets/"))
                        {
                    ...
                    logic.Synchronize(paramsBridge);
        ...
        private void DrawTabGeneral()
        {
            ...
            GUILayout.BeginHorizontal();
            {
                if (GUILayout.Button("SELECT BRIDGE .xml FILE", GUILayout.Height(30)))
                    logic.SelectProjectFolder(paramsBridge);
            }
            ...
            { 
                string path = paramsBridge.IsBridgeFileValid() ? paramsBridge.bridgeFilePath : logic.projectFolderPath;
            ...
            GUILayout.BeginHorizontal();
            {
                paramsBridge.assetName = EditorGUILayout.TextField(new GUIContent("Terrain Asset Name", "Name of the GameObject container that holds your terrain GameObject(s)."),paramsBridge.assetName, GUILayout.ExpandWidth(true));
            }
            GUILayout.EndHorizontal();
            ...
            GUILayout.BeginHorizontal();
            {
                paramsBridge.deleteUnusedAssets = EditorGUILayout.Toggle(new GUIContent("Delete unused Assets", "If enabled automatically cleans up unused terrain assets."), paramsBridge.deleteUnusedAssets);
            }
            GUILayout.EndHorizontal();
            ...
            GUILayout.BeginHorizontal();
            {
                paramsBridge.isImportTextures = EditorGUILayout.Toggle(new GUIContent("Import Layers", "Choose whether terrain layers are automatically imported. \n If 'false' the terrain uses only a simple colormap for texturing."), paramsBridge.isImportTextures);
            }
            GUILayout.EndHorizontal();
            ...
            GUILayout.BeginHorizontal();
            {
                ...
                paramsBridge.userSplit = Mathf.RoundToInt(GUILayout.HorizontalSlider(paramsBridge.userSplit, 0, 5));
                GUILayout.Label((paramsBridge.UserSplit).ToString(), GUILayout.Width(36));            
            }
            GUILayout.EndHorizontal();
            ...
            GUILayout.BeginHorizontal();
            {
                ...
                float oldScale = paramsBridge.worldScale;
                float newScale = GUILayout.HorizontalSlider(paramsBridge.worldScale, 0, 8, GUILayout.ExpandWidth(true));
                if (oldScale != newScale)
                {
                    paramsBridge.worldScaleString = newScale.ToString("#0.00");
                    paramsBridge.worldScale = newScale;
                }

                var oldString = paramsBridge.worldScaleString;
                paramsBridge.worldScaleString = GUILayout.TextField(paramsBridge.worldScaleString, textFieldStyle, GUILayout.Width(50));
                
                if (oldString != paramsBridge.worldScaleString)
                    if (float.TryParse(paramsBridge.worldScaleString, NumberStyles.Any, CultureInfo.InvariantCulture, out float newVal))
                        paramsBridge.worldScale = newVal;
            ...
            GUILayout.BeginHorizontal();
            {
                paramsBridge.materialType = (MaterialType)EditorGUILayout.EnumPopup(new GUIContent("Material Type", "The rendering pipeline for which the terrain material will be set up for. Chose custom to set you own material."),paramsBridge.materialType, GUILayout.ExpandWidth(true));
            }
            GUILayout.EndHorizontal();
            
            if (paramsBridge.layerWarning)
            {
                if (paramsBridge.materialType == MaterialType.URP || paramsBridge.materialType == MaterialType.Standard)
                {
                    GUILayout.Space(spacePixels);

                    GUILayout.Box("Warning - " + (paramsBridge.materialType == MaterialType.URP ? "URP" : "The Built-in Render Pipeline") + " uses additional shader passes with more than 4 terrain layers. This can cause performance issues. To avoid this reduce your material layers in World Creator." , warningStyle);

                    GUILayout.Space(spacePixels);
                }
                else if (paramsBridge.materialType == MaterialType.HDRP)
                {
                    GUILayout.Space(spacePixels);
            ...
            if (paramsBridge.materialType == MaterialType.Custom)
            {
                GUILayout.BeginHorizontal();
                {
                    paramsBridge.customMaterial = EditorGUILayout.ObjectField("", paramsBridge.customMaterial, typeof(Material), false, GUILayout.ExpandWidth(true)) as Material;
                }
                GUILayout.EndHorizontal();
            }
        }
